...
export function configureInitCommand(program) {
    return commonOptions(program
        ...
        .option("-t, --tag <package tag>", "The version of the @trigger.dev/sdk package to install", "latest")
        ...
        .option("--pkg-args <args>", "Additional arguments to pass to the package manager, accepts CSV for multiple args")).action(async (path, options) => {
...
async function _initCommand(dir, options) {
    ...
    log.info("Next steps:");
    log.info(`   1. To start developing, run ${chalk.green(`npx trigger.dev@${options.tag} dev${options.apiUrl === CLOUD_API_URL ? "" : ` -a ${options.apiUrl}`}`)} in your project directory`);
    log.info(`   2. Visit your ${projectDashboard} to view your newly created tasks.`);
    log.info(`   3. Head over to our ${cliLink("v3 docs", "https://trigger.dev/docs/v3")} to learn more.`);
...
async function detectTsConfig(dir, options) {
    return await tracer.startActiveSpan("detectTsConfig", async (span) => {
        try {
            ...
            if (!tsconfigExists) {
                prettyError("No tsconfig.json found", `The init command needs to be run in a TypeScript project. You can create one like this:`, `npm install typescript --save-dev\nnpx tsc --init\n`);
...
async function installPackages(dir, options) {
    return await tracer.startActiveSpan("installPackages", async (span) => {
        ...
        try {
            ...
            if (options.logLevel === "debug") {
                ...
                installSpinner.stop(`Failed to install @trigger.dev/sdk@${options.tag}. Rerun command with --log-level debug for more details.`);
            }
            if (!(e instanceof SkipCommandError)) {
                recordSpanException(span, e);
            }
