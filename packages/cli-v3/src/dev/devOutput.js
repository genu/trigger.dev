...
import { chalkError, chalkGrey, chalkLink, chalkRun, chalkSuccess, chalkTask, chalkWarning, chalkWorker, cliLink, prettyError, prettyPrintDate, } from "../utilities/cliOutput.js";
...
export function startDevOutput(options) {
    ...
    const backgroundWorkerInitialized = (...[worker]) => {
        ...
        logger.log(`${bullet} ${workerStarted} ${runtime} ${arrow} ${workerVersion} ${pipe} ${testLink} ${pipe} ${runsLink}`);
    };
    ...
    const runStarted = (...[worker, payload]) => {
        ...
        logger.log(`${bullet} ${timestampPrefix} ${chalkGrey("->")} ${link} ${pipe} ${workerPrefix} ${pipe} ${taskPrefix} ${pipe} ${runId}`);
    };
    const runCompleted = (...[worker, payload, completion, durationMs]) => {
        ...
        const taskPrefix = chalkTask(execution.task.id);
        const runId = chalkRun(`${execution.run.id}.${execution.attempt.number}`);
        logger.log(`${bullet} ${timestampPrefix} ${chalkGrey("->")} ${link} ${pipe} ${workerPrefix} ${pipe} ${taskPrefix} ${pipe} ${runId} ${pipe} ${resultText} ${elapsedText}${errorText}`);
    };
